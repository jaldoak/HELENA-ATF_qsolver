      MODULE PARAM
      PARAMETER (NRMAX = 1601,  NPMAX = 1026)                            
...
C-----------------------------------------------------------------------
      MODULE COMMAP
      USE PARAM
      DOUBLE PRECISION  CS(NRMMAX),QS(NRMMAX),FS(NRMMAX),
     ...
     >     RHO0, PPER0, PPAR0, DPVOL1(NRMMAX), DPVOLPER1(NRMMAX),
     >     DWK1(NRMMAX), FDF_OUT(NRMMAX)
...
C-----------------------------------------------------------------------
      MODULE COMFAVG
      USE COMDAT
      DOUBLE PRECISION DVDPSI(1001), VDEV_TEMP(1001), FAVG1(1001)
      DOUBLE PRECISION FAVG_FPS(1001), FAVG2A(1001), FAVG3(1001)
      DOUBLE PRECISION FAVG2B(1001), FAVG2C(1001), FAVG2(1001)
      DOUBLE PRECISION DFAVGDPSI(1001), DFAVG1(1001), RES_CONV
      DOUBLE PRECISION DF2AVG(1001)
      INTEGER RES_CHECK, FG_CHECK
      !&      DELFAVG(1001), BFAVG(1001), TPERFAVG(1001), RHOFAVG(1001)
      END
************************************************************************
*DECK HELENA
      PROGRAM HELENA 
...
C------------------------------------------- DEFINE INPUT NAMELISTS ----
     ...
     >                 ATE,BTE,CTE,DTE,ETE,FTE,GTE,HTE,
     >                 ATH,BTH,CTH,DTH,ETH,FTH,GTH,HTH,
     >                 VH,VF2,VTE,VOM2,VTH,QIN,QTEST,FGUESS,
...
C----------------------------------------- REMOVE OPEN STAT. ON IBM ----
      ...
      ! IF ((ICUR.EQ.0).AND.(IGAM.NE.5)) NMESH = 1 !uncomment later DOAK
...
C-------------------------------- INITIAL GUESS FOR A ------------------
      A = 4. * B/DABS(B) 
      IF (IGAM.EQ.5) A = 1.0  !uncomment to use A -> AB qsolver
C------------------------------------ CALCULATE SHAPE OF SOLOVIEV ------
      IF (ISOL.EQ.2) THEN
      A = 2*(1.D0 + (1.D0- EPS**2 /4.)/ELLIP**2)
      B = 2*EPS + TRIA/ELLIP**2/(1.D0+ (1.D0- EPS**2 /4.D0)/ELLIP**2)
C        CALL SOLSHP(FR,MHARM)
      !    CPSURF=.5*EPS**2/(1.D0+EPS**2)**2*ELLIP/DSQRT(1.D0-EPS**2/4.D0)
      !    RADIUS = DSQRT(EPS**2/(1.D0+EPS**2))
      !    B0 = DSQRT(1.D0+EPS**2)
      ...
      ELSEIF (ISOL.EQ.1) THEN
            A = 2*(1.D0 + (1.D0- EPS**2 /4.)/ELLIP**2)
            B = 4*EPS * (1 + TRIA*(1-0.25*EPS**2)/ELLIP**2) / A
      ENDIF
...
C------------------------------------ NORMALIZE PROFILES ---------------
      IF ((IGAM.EQ.2).OR.(IGAM.EQ.3)) THEN
         ...
         DO I=1,NPTS
            VF2(I) = VF2(I) / FSCALE
         ENDDO
      ELSEIF ((IGAM.EQ.5).AND.FGUESS(1).NE.-999) THEN
            FSCALE = FGUESS(1)
      ENDIF
...
C------------------------------------------- LOOP OVER CURRENT PROFILE
      ...
      IF (IGAM.EQ.5) THEN
            ! NR = 50
            ! NP = 32
            ! IF (NRTMP.GT.100) NR = 100
            ! IF (NPTMP.GT.64) NP = 64
      ENDIF
...
C------------------------------------ INITIALIZE KK AND QQ TO ZERO -----
         DO I=1,4*NR*NP
            PSIOLD(I) = PSI(I)
         ENDDO
C------------------------------------ CALCULATE FAVGs and DVDPSI for Q-SOLVER -----
      !  IF ((IGAM.EQ.5).AND.(NI*NMG.EQ.1)) THEN
       IF (((IGAM.EQ.5).OR.(IGAM.LE.0)).AND.(NI*NMG.EQ.1)) THEN
            CALL CALCFAVG(XX,YY,PSI,XAXIS,A,NAX,RAX,SAX,NI,NMG,CX,CY)
       ENDIF
...
C------------------------------------ FIND MAGN. AXIS AND PSI AT AXIS --
        CALL FINDAXIS(XX,YY,NR,NP,PSAXIS,XAXIS,YAXIS,NAX,RAX,SAX,IAS)
        A = A / (1.-PSAXIS)
        IF (IGAM.EQ.5) A = 1.D0 !uncomment for A->AB qsolvers
      !   IF (IGAM.EQ.5) A = DSQRT(XGAMMA(0.D0)) !alt for q-solver
...
C     Q-Solver convergence check for coarse grid (if used)
      WRITE(*,*) 'MAX integral error', RES_CONV/DSQRT(NR-1.D0)
      WRITE(*,*) DABS(XAXISOLD-XAXIS), DABS(YAXISOLD-YAXIS)
      IF (((RES_CONV/DSQRT(NR-1.D0)).LT.1.D-6).AND.(NR.LT.NRTMP)) THEN
            WRITE(*,*)
            WRITE(*,*) 'COARSE_GRAIN CONVERGED:', 
     >       RES_CONV/DSQRT(NR-1.D0)
            WRITE(*,*)
            GOTO 667
         ENDIF
     ...
     >     (DABS(YAXISOLD-YAXIS).LT.1.D-4)) THEN
         ...
         GOTO 999
      ENDIF
C     Q-Solver main convergence check.
      ! IF ((RES_CONV/DSQRT(NR-1.D0)).LT.1.D-12) THEN
      IF ((RES_CONV/DSQRT(NR-1.D0)).LT.1.D-6) THEN
         WRITE(*,*)
         WRITE(*,*) 'Q-SOLVER FLUX INTEGRALS CONVERGED:', 
     >    RES_CONV/DSQRT(NR-1.D0)
         WRITE(*,*)
      !    GOTO 999 !comment this line for consistent q-out
         IF (RES_CHECK.EQ.1) GOTO 999
         IF (NMG.GE.5) RES_CHECK = 1
      ENDIF
      WRITE(*,*) NMG, NMESH
     ...
     >            CX,CY,XAXIS,XAXISOLD,YAXIS,NAX,RAX,SAX,IGAM,IAS,ISOL)
        YAXISOLD = YAXIS
...
C        CALL PLOTGR(0,XX,YY,NR,NP,IAS,40)
     ...
     >        ,CX,CY,IAS,AMPL,SUMDQ)
         ...
         ENDDO
C-------Calculate F(q,psi) for q-solver         
      !   ELSEIF (((IGAM.EQ.5).OR.(IGAM.LE.2)).AND.RES_CHECK.EQ.0) THEN
         ELSEIF ((IGAM.EQ.5).AND.RES_CHECK.EQ.0) THEN
            CALL CALCFAVG(XX,YY,PSI,XAXIS,A,NAX,RAX,SAX,NI,NMG,CX,CY)
            ! DO II = 0, 10
            !       WRITE(*,*) XGAMMA(II*0.1D0)
            ! ENDDO
...
C      IF (NDIAG.EQ.1) write(*,*) ' final remesh'
     ...
     >            CX,CY,XAXIS,XAXISOLD,YAXIS,NAX,RAX,SAX,IGAM,IAS,ISOL)
!       IF ((IGAM.EQ.5).OR.(IGAM.LE.2)) 
!      >   CALL CALCFAVG(XX,YY,PSI,XAXIS,A,NAX,RAX,SAX,NI,NMG,CX,CY)

!       CALL FORMKQ(XX,YY,PSI,NRMAP,NPMAP,QQ,A,B,C,OMGOT,HOT,EPS,IGAM,ITH
!      >        ,ISOL,NI,IAS)
...
C-----BINARY SEARCH DET
     ...
     >        DABS(BTOT - DSQRT(DABS(A)) * TEMP * THETAPS * THTOF)
         ...
         IF (BM3*BM .LT.0) THEN !Should this be BM3*BM0???
            DETL = -0.3
            DETR = 0
            GOTO 40
         ENDIF
...
C---------------------------------
     ...
     >        DABS(BTOT - DSQRT(DABS(A)) * TEMP * THETAPS * THTOF)
         ...
         ENDIF
 10   CONTINUE
c 20   IF (DABS(DETR).EQ.0.8D0) STOP 'MIRROR INSTABILITY OCCURS'
 20   IF ((DABS(DETR).EQ.0.8D0).AND.(THTOF.NE.0)) 
     > STOP 'MIRROR INSTABILITY OCCURS'
c      This line is causing problems - double-check if important later.
c      IF ((DABS(DETR).EQ.0.8D0).AND.(THTOF.EQ.0)) 
c     > WRITE(*,*) 'MIRROR INSTABILITY OCCURS.  THTOF=0 so Continuing.'
      ! IF (IGAM.EQ.5) A = ATMP
      RETURN
      END
************************************************************************
...
C-----Q-solver RJPHI
      IF ((IGAM.EQ.5).OR.(IGAM.LE.2)) THEN
            Q = QVAL(PS)
            FA = FAVGVAL(PS,1)
            FB = FAVGVAL(PS,2)
            FB1 = FAVGVAL(PS,6)
            ...
            DACQ = FAVGVAL(PS,4)
            DFA = FAVGVAL(PS,5)
            ALFAF = EPS**2 * FA * FC / (2*PI*Q)**2
            TESTF = PI / EPS * Q / FC / 0.5 / FACTAS
            F02 = XGAMMA(0.D0)

!             ARHS = Q * DACQ / FC**2 / (1+ALFAF)
!             ARHS = ARHS - B * RHO * DTE / 2. / EPS *
!      >             ((1.+EPS*X)**2 - FB1 / FC / (1+ALFAF))
            ARHS1 = Q * DACQ / FC**2 / (1+ALFAF)
            ARHS1 = ARHS1 + A * B * RHO * DTE *
     >             (FB / FC / (1+ALFAF) - (1.+EPS*X)**2)

            ARHS2 = A * B * RHO * DTE * FB / FC / (1+ALFAF)
            ARHS2 = ARHS2 + DFA / FC / (1+ALFAF)
            ARHS2 = ARHS2 - ALFAF * DF * F / (1+ALFAF)
            ARHS2 = ARHS2 - A * B * RHO * DTTOT *
     >             (1.+EPS*X)**2
c-----correction
!             ARHS2 = A * B * RHO * DTE * FB / FC / (1+ALFAF) * EPS**2
!             ARHS2 = ARHS2 + DFA / FC / (1+ALFAF)
!             ARHS2 = ARHS2 - ALFAF * DF * F / (1+ALFAF)
!             ARHS2 = ARHS2 - A * B * RHO * DTTOT *
!      >             (1.+EPS*X)**2
...
C-----ANISOTROPY ARHS
            ARHS3 = - A*B*(1.+EPS*X)**2 *(DTE + DH*HOT)
     >            * (RHO - EPS**2 / (1+ALFAF)*FB1/FC)
            ARHS3 = ARHS3 - A*B*(1.+EPS*X)**2 *DOMG*OMGOT*0.5
     >            * (RHO*(1+EPS*X)**2 - EPS**2 / (1+ALFAF)*FB2/FC)
            ARHS3 = ARHS3 + A*B*(1.+EPS*X)**2
     >            * (RHO*DWDPSI - EPS**2/(1+ALFAF)*FB3/FC)
            ARHS3 = ARHS3 - ALFAF*DF*F/(1+ALFAF)*EPS**2/(1-DET)
            ARHS3 = ARHS3 + DFA/FC/(1+ALFAF)*EPS**2/(1-DET)

C-----As of Jan2025, best version:
!             ARHS4 = - A*B*(1.+EPS*X)**2 *(DTE + DH*HOT)
!      >            * (RHO - 1.D0/(1+ALFAF)*FB1/FC/(1+EPS*X)**2)
!             ARHS4 = ARHS4 - A*B*(1.+EPS*X)**2 *DOMG*OMGOT*0.5
!      >            * (RHO*(1+EPS*X)**2 
!      >            - 1.D0/(1+ALFAF)*FB2/FC/(1+EPS*X)**2)
!             ARHS4 = ARHS4 + A*B*(1.+EPS*X)**2
!      >            * (RHO*DWDPSI - 1.D0/(1+ALFAF)*FB3/FC/(1+EPS*X)**2)
!             ARHS4 = ARHS4 - ALFAF*DF*F/(1+ALFAF)/(1-DET)
!             ARHS4 = ARHS4 + DFA/FC/(1+ALFAF)/(1-DET)
C-----Variation with (1-DET) correction
            ARHS4 = - A*B*(1.+EPS*X)**2 *(DTE + DH*HOT)
     >            * (RHO - 1.D0/(1+ALFAF)*FB1/FC/(1+EPS*X)**2
     >              /(1-DET))
            ARHS4 = ARHS4 - A*B*(1.+EPS*X)**2 *DOMG*OMGOT*0.5
     >            * (RHO*(1+EPS*X)**2 
     >            - 1.D0/(1+ALFAF)*FB2/FC/(1+EPS*X)**2/(1-DET))
            ARHS4 = ARHS4 + A*B*(1.+EPS*X)**2
     >            * (RHO*DWDPSI - 1.D0/(1+ALFAF)*FB3/FC/(1+EPS*X)**2
     >              /(1-DET))
            ARHS4 = ARHS4 - ALFAF*DF*F/(1+ALFAF)/(1-DET)
            ARHS4 = ARHS4 + DFA/FC/(1+ALFAF)/(1-DET)
C-----Variation for A scaling
!             ARHS4 = - B*(1.+EPS*X)**2 *(DTE + DH*HOT)
!      >            * (RHO - 1.D0/(1+ALFAF)*FB1/FC/(1+EPS*X)**2
!      >              /(1-DET))
!             ARHS4 = ARHS4 - B*(1.+EPS*X)**2 *DOMG*OMGOT*0.5
!      >            * (RHO*(1+EPS*X)**2 
!      >            - 1.D0/(1+ALFAF)*FB2/FC/(1+EPS*X)**2/(1-DET))
!             ARHS4 = ARHS4 + B*(1.+EPS*X)**2
!      >            * (RHO*DWDPSI - 1.D0/(1+ALFAF)*FB3/FC/(1+EPS*X)**2
!      >              /(1-DET))
!             ARHS4 = ARHS4 - ALFAF*DF*F/(1+ALFAF)/(1-DET)/F02
!             ARHS4 = ARHS4 + DFA/FC/(1+ALFAF)/(1-DET)/F02


            ALFAF = ALFAF / (2*PI)**2
            QA = DACQ/(1+ALFA)/FC**2
            QB = EPS*RHO*(1+EPS*X)**2 *FA*FC/(2*PI)**4/Q**2 /(1+ALFAF)
            QB = QB + (RHO*(1+EPS*X)**2 - FB1/(1-DET)/FC)
     >           /EPS/(1+ALFAF)
            QC = EPS*RHO*(1+EPS*X)**2 *FA*FC/(2*PI)**4 /Q**2/(1+ALFAF)
            QC = QC + (RHO*(1+EPS*X)**2 - FB2/(1+EPS*X)**2 /(1-DET)/FC)
     >           /EPS/(1+ALFAF)
            QE = (FB3 - RHO*DWDPSI)/(1-DET)/(1+EPS*X)**2 
     >           /(1+ALFAF)
            
            ! ARHS4 = QA/(1-DET) - A*B*EPS*QB*(DTTOT+DH*HOT)
            ! ARHS3 = ARHS3 - A*B*(1+EPS*X)**2 *EPS*DOMG*OMGOT*0.5*QC
            ! ARHS3 = ARHS3 - A*B*(1+EPS*X)**2 *QE

            ARHS5 = QA - A*B*EPS*QB*(DTTOT)

!             WRITE(60,*) X,PS,ARHS,ARHS2/A,ARHS3/A,ARHS4/A,FB,FB1,FB2,
!      >       FB3, DWDPSI


!             IF ((IGAM.EQ.5).AND.(RES_CHECK.EQ.0).AND.(FG_CHECK.EQ.0)) 
!      >       ARHS = ARHS4
            IF ((IGAM.EQ.5).AND.(FG_CHECK.EQ.0)) 
     >       ARHS = ARHS4
!             IF ((IGAM.EQ.5).AND.(FG_CHECK.EQ.0).AND.(NMG*NI.EQ.1)) 
!      >       ARHS = ARHS2

      ENDIF

!       WRITE(60,*) X, PS, ARHS, DF*F, F, DF, DTTOT, 
!      >       -1.D0 * DTTOT * B * RHO * (1.+EPS*X)**2
      ! WRITE(60,*) X,PS,ARHS,ARHS1/A, ARHS2/A
!       FDF_OUT = -(1.D0 - DET) * (ARHS + DTTOT * B * RHO * 
!      >       (1.+EPS*X)**2)
...
C      WRITE(*,*)'DWDPSI DH DTE DTTOT', DWDPSI,DH,DTE,DTTOT
     ...
     >   HOT*DTE*H/TEMP - 0.5*(1+EPS*X)**2*OMGOT*OMG*DTE/TEMP) * RHO0
      ...
      ! IF (IGAM.EQ.5) THEN
      !       ARHS = -ARHS * (1+EPS*X)**2 - DF * F / (1.D0 - DET) / A**2
      ! ENDIF
...
************************************************************************
*DECK CALCFDF_OUT
      SUBROUTINE CALCFDF_OUT(X,PS,SUMDPSI,A,FDF_OUT1,DET)
c-----------------------------------------------------------------------
c FUNCTION TO CALCULATE R * JPHI(PSI,X) (=LAPLACE(*)PSI)
C PHYSICS IS SPECIFIED HERE
C INPUT SUMDPSI = |GRAD(PSI)|
...
!      >       (1.+EPS*X)**2)
      ...
      !FDF_OUT value to be used in MAPPING
      FDF_OUT1 = -(1.D0 - DET) * (ARHS + DTTOT * B * RHO * 
     >       (1.+EPS*X)**2)
...
C           CALCRJPHI(X,PS,SUMDPSI,A,ARHS,DET)
            ...
            ARHS = A * ARHS / (1.+EPS*X)
            IF (ISOL.GE.1) ARHS = A * (1.+ B*X*(1.+EPS*X/2.))/(1.+EPS*X)
            ! WRITE(61,*) N, N1, XX(2,N3), X, PS, ARHS
            ! WRITE(61,*) X, PS, ARHS
...
c            WRITE(20,*)ARHS,PS,X
            ! WRITE(61,*) X, PS, ARHS
            ! WRITE(60,*) N, N1, XX(2,N3), X, PS, ARHS
C------------------------------------- 4 NODES PER ELEMENT ------------
            DO 180 I=1,4
C------------------------------------- 4 FUNCTIONS V PER NODE ---------
              DO 190 J=1,4
...
C------------------------------------- 4 FUNCTIONS H IN PSI -----------
  ...
  190         CONTINUE
  180       CONTINUE
            ! IF (PS.LT.1.D-3) WRITE(60,*) X, PS, ARHS, QQ(NROW)
  170     CONTINUE
  160   CONTINUE
  150 CONTINUE
      IF (ITH.GE.1) THEN
...
************************************************************************
*DECK INIFQ
      SUBROUTINE INIFQ(NPTS, NPT, VPSI, VDATA, VINT, VDEV)
C------------------------------------------------------------------
C INTERPOLATE ON DISCRETE DATA (WITH A PSI ARRAY)
C FOR PROFILE INITIALIZING
C USED TO PRODUCE GINT FROM Q IN THE Q-SOLVER OPTION
C
C INPUT  :
C     NTP   NUMBER OF NODES ON OUTPUT
C     NTPS  NUMBER OF NODES ON INPUT
C     VPSI  INPUT PSI ARRAY
C     VDATA DATA
C OUTPUT :
C     VINT  INTERPOLATED DATA
C     VDEV  INTERPOLATED DERIVATIVE
C------------------------------------------------------------------
      USE PARAM
      IMPLICIT DOUBLE PRECISION (A-H, O-Z)
      INTEGER NPTS, NPT
      DOUBLE PRECISION VPSI(*), VDATA(*), VINT(*), VDEV(*)
      DOUBLE PRECISION PSINODE(NPTSMAX),PSIDEV(NPTSMAX)
      DOUBLE PRECISION VNEW(NPTSMAX)
      DOUBLE PRECISION DD1(NPTSMAX),DD2(NPTSMAX),DD3(NPTSMAX)
     >                ,DD4(NPTSMAX)
     ...
     >                ,DV4(NPTSMAX)
      ...
      CALL BUBBLESORT(NPTS,VPSI,VDATA)
...
!          ENDIF
!       ENDDO
C Take diff at boundaries.
      PSIDEV(1) = (VDATA(2) - VDATA(1)) / (VPSI(2) - VPSI(1))
      PSIDEV(NPTS) = (VDATA(NPTS) - VDATA(NPTS-1)) 
     >  / (VPSI(NPTS) - VPSI(NPTS-1))

c            CALL SPLINE(NPTS,PSINODE,PSIDEV,0.D0,0.D0,2,DV1,DV2,DV3,DV4)
      ! DO I = 1, NPTS
      !       WRITE(*,*) I, VPSI(I), VDATA(I)
      ! ENDDO
      CALL SPLINE(NPTS,VPSI,VDATA,PSIDEV(1),PSIDEV(NPTS),1
     >      ,DD1,DD2,DD3,DD4)
      DO I = 1, NPT
            PSI = REAL(I-1)/REAL(NPT-1)
            DATAPSI = SPWERT(NPTS,PSI,DD1,DD2,DD3,DD4,VPSI,ALG)
c$$$         DEV1 = SPWERT(NPTS,PSI,DV1,DV2,DV3,DV4,PSINODE,ALG)
            VINT(I) = DATAPSI
            VDEV(I) = ALG(1)
      ENDDO
      ! DO I = 1, NPT
      !       WRITE(*,*) I, VINT(I), VDEV(I)
      ! ENDDO
      ! WRITE(*,*) PSIDEV(1), PSIDEV(NPTS)
      RETURN
      END
************************************************************************
*DECK DPDPSI
...
! C-----------QINT testing for q-solver
!             CALL ININUM(NPTS, NPT, VF2, QINT, DQIN)
!             NPT_SPLINE = 101
!             DO I = 1,NPT
!                PSI_SPLINE(I) = (DFLOAT(I-1)/DFLOAT(NPT-1))
!             ENDDO
!             CALL SPLINE(NPT_SPLINE,PSI_SPLINE,VF2,DQIN(1),DQIN(NPT),
!      >         1,DF1,DF2,DF3,DF4)
...
! C-----------QINT testing for q-solver with IGAM=2
            CALL ININUM(NPTS, NPT, QTEST, QINT, DQIN)

c-----Option for q-solver input-------
      ELSEIF (IGAM.EQ.5) THEN
         CALL ININUM(NPTS, NPT, VF2, QINT, DQIN)
         NPT_SPLINE = 101
         DO I = 1,NPT
            PSI_SPLINE(I) = (DFLOAT(I-1)/DFLOAT(NPT-1))
         ...
         CALL SPLINE(NPT_SPLINE,PSI_SPLINE,VF2,DQIN(1),DQIN(NPT),
     &         1,DF1,DF2,DF3,DF4)
c-----FGUESS Initialisation
         IF (FGUESS(1).NE.-999.) THEN
         CALL ININUM(NPTS, NPT, FGUESS, GINT, DGAM)
         DO I = 1, NPT
            GINT(I) = DSQRT(GINT(I))
            DGAM(I) = 0.5D0 * DGAM(I)/ GINT(I)
           IF (GINT(I).EQ.0.D0) DGAM(I) = 0.D0
         ...
         FG_CHECK = 1
         ENDIF
      ELSEIF (IGAM.EQ.6) THEN
         DO 11 I=1,NPT
            PSI = REAL(I-1)/REAL(NPT-1)
            GAMPSI   = HGA + AGA*PSI + BGA*PSI**2 + CGA*PSI**3 
     >           + DGA*PSI**4
     >           + EGA*PSI**5 + FGA*PSI**6 + GGA*PSI**7
            DGDPSI   = AGA + 2*BGA*PSI + 3*CGA*PSI**2
     >           + 4*DGA*PSI**3 + 5*EGA*PSI**4 + 6*FGA*PSI**5
...
*DECK QVAL
      FUNCTION QVAL(PSI)
C------------------------------------------------------------------
C THE Q-PROFILE VALUE FOR A GIVEN PSI INPUT. (FOR THE Q-SOLVER OPTION)
C------------------------------------------------------------------
      USE PARAM
      USE COMDAT
      USE COMPROF
      ...
      IF (PSI.GE.1.) THEN
         QVAL = QINT(NPT)
      ELSE
         QVAL = QINT(NINT) + ((ZPSI)-DPS*(NINT-1))/DPS *
     >        (QINT(NINT+1)-QINT(NINT))
      ENDIF
...
C------------------------------------------------------------------
      ...
      IF (PSI.GE.1.) THEN
         DQDPSI = DQIN(NPT)
      ELSE
         DQDPSI = DQIN(NINT) + ((ZPSI)-DPS*(NINT-1))/DPS *
     >        (DQIN(NINT+1)-DQIN(NINT))
      ENDIF
      RETURN
      END
************************************************************************
...
************************************************************************
*DECK FAVGVAL
      FUNCTION FAVGVAL(PSI,FVAL)
C------------------------------------------------------------------
C THE FLUX-AVERAGED TERMS FOR A GIVEN PSI INPUT (FOR THE Q-SOLVER OPTION)
C------------------------------------------------------------------
      USE PARAM
      USE COMDAT
      USE COMPROF
      ...
      IF (PSI.GE.1.) THEN
         IF (FVAL.EQ.1) THEN
            FAVGVAL = FAVG1(NPT)
         ELSEIF (FVAL.EQ.2) THEN
            FAVGVAL = FAVG2(NPT)
         ELSEIF (FVAL.EQ.3) THEN
            FAVGVAL = FAVG3(NPT)
         ELSEIF (FVAL.EQ.4) THEN
            FAVGVAL = DFAVGDPSI(NPT)
         ELSEIF (FVAL.EQ.5) THEN
            FAVGVAL = DFAVG1(NPT)
         ELSEIF (FVAL.EQ.6) THEN
            FAVGVAL = FAVG2A(NPT)
         ELSEIF (FVAL.EQ.7) THEN
            FAVGVAL = FAVG2B(NPT)
         ELSEIF (FVAL.EQ.8) THEN
            FAVGVAL = FAVG2C(NPT)
         ELSEIF (FVAL.EQ.9) THEN
            FAVGVAL = DF2AVG(NPT)
         ENDIF   
      ELSE
         IF (FVAL.EQ.1) THEN
            FAVGVAL = FAVG1(NINT) + ((ZPSI)-DPS*(NINT-1))/DPS *
     >        (FAVG1(NINT+1)-FAVG1(NINT))
         ELSEIF (FVAL.EQ.2) THEN
            FAVGVAL = FAVG2(NINT) + ((ZPSI)-DPS*(NINT-1))/DPS *
     >        (FAVG2(NINT+1)-FAVG2(NINT))
         ELSEIF (FVAL.EQ.3) THEN
            FAVGVAL = FAVG3(NINT) + ((ZPSI)-DPS*(NINT-1))/DPS *
     >        (FAVG3(NINT+1)-FAVG3(NINT))
         ELSEIF (FVAL.EQ.4) THEN
            FAVGVAL = DFAVGDPSI(NINT) + ((ZPSI)-DPS*(NINT-1))/DPS *
     >        (DFAVGDPSI(NINT+1)-DFAVGDPSI(NINT))
         ELSEIF (FVAL.EQ.5) THEN
            FAVGVAL = DFAVG1(NINT) + ((ZPSI)-DPS*(NINT-1))/DPS *
     >        (DFAVG1(NINT+1)-DFAVG1(NINT))
         ELSEIF (FVAL.EQ.6) THEN
            FAVGVAL = FAVG2A(NINT) + ((ZPSI)-DPS*(NINT-1))/DPS *
     >        (FAVG2A(NINT+1)-FAVG2A(NINT))
         ELSEIF (FVAL.EQ.7) THEN
            FAVGVAL = FAVG2B(NINT) + ((ZPSI)-DPS*(NINT-1))/DPS *
     >        (FAVG2B(NINT+1)-FAVG2B(NINT))
         ELSEIF (FVAL.EQ.8) THEN
            FAVGVAL = FAVG2C(NINT) + ((ZPSI)-DPS*(NINT-1))/DPS *
     >        (FAVG2C(NINT+1)-FAVG2C(NINT))
         ELSEIF (FVAL.EQ.9) THEN
            FAVGVAL = DF2AVG(NINT) + ((ZPSI)-DPS*(NINT-1))/DPS *
     >        (DF2AVG(NINT+1)-DF2AVG(NINT))
         ENDIF 
      ENDIF
...
C------------------------- UPDATE OLD MESH FOR THE FIRST ITERATION ----
    ...
    8 CONTINUE
      ...
      !     WRITE(60,*) XX(K,I), YY(K,I), PSI(4*(I-1)+K)
   20   CONTINUE
   10 CONTINUE
...
C---IN THIS CASE WE'LL RETAIN CX AND CY VALUE ON AXIS
      ...
      IF (ISOL.GE.1) ARHS = (1.+ B*X*(1.+EPS*X/2.))
...
c      CALL PRARR1('PSI : ',PSI,4*NRNEW*NPNEW,203)
      ...
      DO 11 I = 1,NR*NP
        DO 21 K=1,4
          ...
          YYOLD(K,I) = YY(K,I)
          PSIOLD(4*(I-1)+K) = PSI(4*(I-1)+K)
      !     WRITE(62,*) XX(K,I), YY(K,I), PSI(4*(I-1)+K)
   21   CONTINUE
   11 CONTINUE
...
C-----------------------------------------------------------------------
     ...
     >                  FPLOT(2*NRMMAX), DWKPLOT(2*NRMMAX),
     >                  FDFPLOT(2*NRMMAX)
...
C-------CALCULATE CHI BASED ON INTERGRATION
     ...
     >                  Y,YR,YS,YRS,YRR,YSS)
            CALL INTERP(PSI(4*(N1-1)+1),PSI(4*(N2-1)+1),
     >       PSI(4*(N3-1)+1),PSI(4*(N4-1)+1),
     >       -1.D0,S,PS,PSR,PSS,PSRS,PSRR,PSSS)
            CALL INTERP2(PSI(4*(N1-1)+1),PSI(4*(N2-1)+1),PSI(4*(N3-1)+1)
     ...
     >                PSI(4*(N4-1)+1),-1.D0+DR,S,X1,XR1,XS1,YR1,YS1,PS1)
            ...
            RHO1 = CALCRHO(X, PS)
            PRESS1 = RHO1 * TPER * B * A
            PRESSPER1 = RHO1 * TPER**2 / TEMS(PS) * B * A
            DETR = (DET1 - DET) /  DR
            ...
            ! BOUNDLEN = BOUNDLEN+BIGR*SUMDPSI/DABS(PSIR)*EJAC*WGAUSS(K)
            ! ZJDCHI = BIGR * EJAC1 / ABS(PSR1)  
            ! DL = SQRT(XS1**2 + YS1**2)
            ! IF (IAV.EQ.0) THEN
            !       DLV = DL
            ! ELSE
            !       DLV = ZJDCHI
            ! ENDIF
            ! BOUNDLEN = BOUNDLEN + XGAUSS(K) * DLV
     ...
     >              *WGAUSS(K)
            ! SUMJ = SUMJ + ARHS*BIGR*SUMDPSI/DABS(PSIR)*EJAC*WGAUSS(K)
            PVOL1 = PVOL1  + WGAUSS(K) * EJAC * PRESS1 * (1.D0+EPS*X)
            PVOLPER1  = PVOLPER1  + WS * EJAC * PRESSPER1 * (1.D0+EPS*X)
...
c-----------<jphi> calculation
            ...
            SUMJ = SUMJ + 
     >       (PSIXX + PSIYY - EPS * PSX0 / (1+EPS * X))/(1+EPS*X)
     >       *XLEN*WS
            CALL CALCFDF_OUT(X, PS, SUMDPSI, A, FDF_OUT1, DET)
            SUMFDF = SUMFDF + FDF_OUT1 *XLEN*WS 
!             SUMJ = SUMJ + XGAUSS(K) *(PSIXX + PSIYY - EPS * PSX0 / 
!      >      (1+EPS * X))/(1+EPS*X) * DLV

!             WRITE(61,*) I, PS, SUMQ, SUMQR, EJAC, BIGR, PSIR,
!      >        PSIRR, ER, XR
C            IF ((K.EQ.1).AND.(J.EQ.1)) WRITE(*,*) XRS, YRS
            ! IF (I.EQ.50) WRITE(*,*) SUMQ, DABS(PSIR), EJAC
            ! IF (I.EQ.50) WRITE(*,*) J, X, N3, XX(2,N3)
   30     CONTINUE
...
c-------QS = EPS * F * CCHI(1,*)
        FPS0 = QVAL(0.D0) * DSQRT(CX*CY) * (1.+EPS*XAXIS)
     >       * 2 / EPS
      !   WRITE(*,*) NR-I+1, FPS0, XGAMMA(0.D0) * DSQRT(DABS(A))
        ...
        FS(NR-I+1) =  QVAL(PS)/0.5/FACTAS/SUMQ/EPS
        BOUNDLEN = DABS(BOUNDLEN)
        CURJ(NR-I+1) = DABS(SUMJ) / BOUNDLEN
      !   FDF_OUT(NR-I+1) = DABS(SUMFDF) / BOUNDLEN
        FDF_OUT(NR-I+1) = SUMFDF / BOUNDLEN
        DPVOL1(NR-I+1) = FACTAS * DABS(PVOL1)
        DPVOLPER1(NR-I+1) = FACTAS * DABS(PVOLPER1)
        DWK1(NR-I+1) = (DPVOL1(NR-I+1) + 2*DPVOLPER1(NR-I+1))*2*PI/2

        !DOAK output
!         WRITE(*,*) NR-I+1, PI*QVAL(PS)/EPS/SUMQ/0.5/FACTAS,
!      >            SQRT(DABS(A))*XGAMMA(PS)
      !   WRITE(*,*)  PS, QS(NR-I+1) / PI
      !   WRITE(62,*) I, PS, SUMQ, QS(NR-I+1), DQS(NR-I+1), SUMQR, PSIR
!         WRITE(*,*) NR-I+1, PS, SUMQR, DQS(NR-I+1)/PI, 
!      >   SQRT(DABS(A))*DGDPSI(PS),SQRT(DABS(A))*XGAMMA(PS)
   10 CONTINUE
      CALL CALCBTPD(XAXIS,0.D0,0.D0,A,BTOT0,TPER0,DET0)
...
c$$$      WRITE(*,*)'cxcy ', CX, CY
      ...
      FS(1) = FPS0 / PI
      FPS0 = QS(1) * DSQRT(CX*CY) * (1.+EPS*XAXIS)
     >       * 2 / EPS / PI
      WRITE(*,*) 'FPS0', FPS0
      FPS0 = QVAL(0.D0) * DSQRT(CX*CY) * (1.+EPS*XAXIS)
     >       * 2 / EPS / PI
      WRITE(*,*) 'FPS0', FPS0
      WRITE(*,*) QVAL(0), CX, CY, (1.+EPS*XAXIS)
      CALL CALCRJPHI(XAXIS, 0.D0, 0.D0, A, ARHS, DET)
      ARHS = A*ARHS/(1.D0+EPS*XAXIS)
      CALL CALCFDF_OUT(XAXIS, 0.D0, 0.D0, A, FDF_OUT0, DET)
      ! CURJ(1) = ARHS
      CURJ(1) = SUMJ0
      FDF_OUT(1) = FDF_OUT0
      DPVOL1(1) = 0.D0
      ...
      ! WRITE(*,*)  0.D0, QS(1) / PI
      !DOAK output
      ! WRITE(62,*) I, 0.D0, SUMQ, QS(1), DQS(1), SUMQR, PSIR

c$$$      WRITE(*,*)'Q0,DET0    :',QS(1)/PI, DET0
      WRITE(22,*) QS(1) / PI
      ! WRITE(*,*) NR-I+1, QS(1) / PI
      DQS(1) = 0.
      DO 40 I = 1, NR-1
        DO 50 J = 1, NP
          DUM = CCHI(1,I*NP)
...
c         DQS(I) = DQS(I) / PSIR * 2 * DSQRT(ZPSI)
        ...
        DQS(I) = DQS(I) / PI
        DQPLOT(NR+1-I) = DQS(I)
        CUPLOT(NR+1-I) = CURJ(I)
        FDFPLOT(NR+1-I) = FDF_OUT(I)
        DWKPLOT(NR+1-I) = DWK1(I)
   45 CONTINUE
      QS0 = QS(1)
      QAXIS = QS(1)
      WRITE(20,*)
...
C-------RIGHT----
     ...
     >       AJPAR,AJPER,AJMAG, APARDEV, APERDEV)
        ...
        FPLOT(2*NR-I) = FPLOT(I)
        DQPLOT(2*NR-I) = DQPLOT(I)
        CUPLOT(2*NR-I) = CUPLOT(I)
        FDFPLOT(2*NR-I) = FDFPLOT(I)
        DWKPLOT(2*NR-I) = DWKPLOT(I)
  100 CONTINUE
...
c      WRITE(35,*)'#  X  Q  DQ/DS  PSI  RHO  P(PAR)  P(PER)  DET JPHI'
      DO I = 1, NR * 2-1
!       IF (IGAM.NE.5)
!      >     FPLOT = XGAMMA(PSIPLOT(I))*DSQRT(DABS(A))/(1-DETPLOT(I))
     ...
     >              BPPLOT(I), CUPLOT(I), DWKPLOT(I), FDFPLOT(I)
...
C-----------------------------------------------------------------------
      ...
      DOUBLE PRECISION XBOUNDLEN(NRMAX), JPHIAVG(NRMAX)
...
c            BOUNDLEN = BOUNDLEN + XLEN
     ...
     >              PSI(4*(N4-1)+1),R,S,X,XR,XS,YR,YS,PS)
               ...
               JPHIAVG(J) = JPHIAVG(J) + ARHS * XLEN * WS
 30         CONTINUE
 20      CONTINUE
         BOUNDLEN = DABS(BOUNDLEN)
         IF (THTOF.GT.0) THEN
         ...
         JPHIAVG(J) = DABS(JPHIAVG(J)) / BOUNDLEN
 ...
 10   CONTINUE
     ...
     >        DETAVG(J), BPHIAVG(J), XBOUNDLEN(J), FAVGVAL(PS,1),
     >        FAVGVAL(PS,2), FAVGVAL(PS,3), FAVGVAL(PS,6),
     >        FAVGVAL(PS,7), FAVGVAL(PS,8), FAVGVAL(PS,4),
     >        FAVGVAL(PS,5), XGAMMA(PS), FAVGVAL(PS,9)
         ELSE
         WRITE(37,*) PS, RHOAVG(J) , PPARAVG(J), PSTARAVG(J), 
     >        DETAVG(J), BPHIAVG(J), XBOUNDLEN(J), JPHIAVG(J)
         ENDIF
 40   CONTINUE
      RETURN
      END
************************************************************************
*DECK CALCFAVG
      SUBROUTINE CALCFAVG(XX,YY,PSI,XAXIS,A,NAX,RAX,SAX,NI,NMG,CX,CY)
C-----------------------------------------------------------------------
C   FLUX SURFACE AVERAGE QUANTITIES
C   Calculate flux-average quantities for q-solver RHS at start of Picard
C   iteration step.   Perform DELTA-loop iteration during this step.
C   TSTARAVG = <0.5(TPER + TPAR)>
C   PSTARAVG = <(2PPER + PPAR)/3>
C   DETAVG   = <DET>
C-----------------------------------------------------------------------
      ...
      DOUBLE PRECISION XX(4,*), YY(4,*), PSI(*)
      ! DOUBLE PRECISION DETAVG(4*NRMAX), TSTARAVG(4*NRMAX)
      ! DOUBLE PRECISION PSTARAVG(4*NRMAX)
      ! DOUBLE PRECISION BPHIAVG(4*NRMAX), RHOAVG(4*NRMAX)
      ! DOUBLE PRECISION PPARAVG(4*NRMAX)
      ! DOUBLE PRECISION XBOUNDLEN(4*NR)
      ! DOUBLE PRECISION BFAVG(4*NP)
      ! DOUBLE PRECISION TPERFAVG(4*NP), RHOFAVG(4*NP)
      DOUBLE PRECISION FAVG1_SP(4*NR), FPS_SP(4*NR), FPS2(4*NR)
      DOUBLE PRECISION FDF_SP(4*NR)
      DOUBLE PRECISION DF_SP(4*NR), SUMQR_SP(4*NR)
      DOUBLE PRECISION FAVG2A_SP(4*NR), FAVG3_SP(4*NR), FPS3(4*NR)
      DOUBLE PRECISION FAVG2B_SP(4*NR), FAVG2C_SP(4*NR),FAVG2_SP(4*NR)
      DOUBLE PRECISION DFAVGDPSI_SP(4*NR), FPS_OLD(4*NR)
      DOUBLE PRECISION F1_OLD(4*NR), F2_OLD(4*NR), F3_OLD(4*NR)
      DOUBLE PRECISION F2A_OLD(4*NR), F2B_OLD(4*NR), F2C_OLD(4*NR)
      DOUBLE PRECISION PSAVG(4*NR), PSAVG2(4*NR), PSAVG3(4*NR)
      DOUBLE PRECISION FAVG1_SP2(4*NR), FAVG1_SP3(4*NR)
      DOUBLE PRECISION FAVG2_SP2(4*NR), FAVG2_SP3(4*NR)
      DOUBLE PRECISION FAVG3_SP2(4*NR), FAVG3_SP3(4*NR)
      DOUBLE PRECISION DFAVGDPSI_SP2(4*NR), DFAVGDPSI_SP3(4*NR)
      DOUBLE PRECISION PSAVG4(4*NR), PSAVG5(4*NR)
      DOUBLE PRECISION DETP(4*NP), BM0P(4*NP), BM3P(4*NP), BMP(4*NP)
      DOUBLE PRECISION BLP(4*NP),BRP(4*NP)
      DOUBLE PRECISION DETLP(4*NP), DETRP(4*NP)

      WRITE(*,*) 'Begin q-solver'

      PI = 2.*DASIN(1.D0)
      FACTAS = 2.
      IF (IAS.EQ.1) FACTAS=1.

C-----Evaluate FPS on the magnetic axis
      IF (NI*NMG.NE.1) THEN 
      ! CALL FINDAXIS(XX,YY,NR,NP,PSAXIS,XAXIS,YAXIS,NAX,RAX,SAX,IAS)
      N1 = NODENO(NAX,1)
      N2 = NODENO(NAX,2)
     ...
     >            R,S,EQPSI,PSIR,PSIS,PSRS,PSRR,PSSS)
      ...
      ! CY = PSIYY0/2.
      CALL CALCBTPD(XAXIS, 0.D0, 0.D0, A, BTOT0, TPER0, DET0)
      FPS0 = QVAL(0.D0) * (1.D0-DET0) * DSQRT(CX*CY) * (1.+EPS*XAXIS)
     >       * 2 / EPS
      F20 = PI / DSQRT(CX*CY) * (1.+EPS*XAXIS) / FACTAS
      F30 = PI / DSQRT(CX*CY) / (1.+EPS*XAXIS) / FACTAS / (1.0-DET)
      ! WRITE(*,*) FPS0, F20, F30, CX, CY, NI
      ! FPS0 = 26.265112204212002
      ! F20 = 1.0893151887072374
      ! F30 = 1.0843818176704787
      ENDIF
      J1 = 1
      J2 = 1
C------------------------------------------------------------------------

C-----Begin flux-surface iterations
      DO 10 J=1, NR-1
	! DO 15 NGR = 1, 4
      ! JNGR = NR+1-J
      JNGR = NR-J
      ...
      ZPSI1 = PSIKN(J)
      PS1 = ZPSI
      PSIR1  = - DPSIKN(J)  /(2.*REAL(NR-1))
      PSIS1  = 0.
      PSIRR1 =   DDPSIKN(J) /(2.*REAL(NR-1))**2

      ZPSI = PSIKN(J)
      PS = ZPSI
      PSIR  = - DPSIKN(J)  /(2.*REAL(NR-1))
      PSIS  = 0.
      PSIRR =   DDPSIKN(J) /(2.*REAL(NR-1))**2
      PSRR = PSIRR

C-----Perform DELTA-loop to calculate variables. on each surface
C-----BINARY SEARCH DET (iterate for DELTA.  Initial guess with DEL=0)
      DO 21 I = 1, NP-1
            ! N1 = (J-1)*NP + I
            ! N2 = (J-1)*NP + I + 1
            ! N3 = (J-1)*NP + I + 1 + NP
     ...
     >            PSI(4*(N4-1)+1),R,S,X,XR,XS,YR,YS,PS)            
            ...
            ! PYJAC = PSR*YS - PSS*YR
            ! PXJAC = PSR*XS - PSS*XR
            PYJAC = PSIR*YS - PSIS*YR
            PXJAC = PSIR*XS - PSIS*XR
            PSX = PYJAC / XJAC
            PSY = -PXJAC / XJAC
            XLEN = DSQRT(XS**2 + YS**2) * (1+EPS*X)
            BOUNDLEN = BOUNDLEN + XLEN * WS
            ...
            ! IF (THTOF.EQ.0) THEN
                  ...
                  PPER = RHO * TPER * A * B
                  PPAR = RHO * TPAR * A * B
                  OMG1 = OMGS(PS) !name1 to avoid COMPROF overlap
                  DTE = DTEDPSI(PS)
                  DTPER = DTE
                  DH = DPDPSI(PS)
                  DOMG1 = DOMDPSI(PS)
                  DTOT = DTE * TPER - DTPER * TPAR
                  DTOT = DTOT / TPER**2
                  DWDPSI = DTE*DLOG(RHO*TPAR/TPER) + TPER * DTOT
            ELSE
                  CALL CALCBTPD(X, PS, SUMDPSI, A, BTOT, TPER, DET)
                  TPAR = TEMS(PS)
                  RHO = TPER / TPAR * CALCRHO(X, PS)
                  ...
                  DH = DPDPSI(PS)
                  DOMG1 = DOMDPSI(PS)
                  BMTT = BTOT - DSQRT(DABS(A)) * TPAR * THE* THTOF
                  DTPER = DTE*DABS(BMTT) + THTOF *
     >       DSQRT(DABS(A))*(DTE*THE+TPAR*DTH)*DSIGN(TPAR,BMTT)
                  DTPER = DTPER / BMTT**2 * BTOT
                  DTOT = DTE * TPER - DTPER * TPAR
                  DTOT = DTOT / TPER**2
                  DWDPSI = DTE*DLOG(RHO*TPAR/TPER) + TPER * DTOT
            ...
            FAVG1_GRID = SUMDPSI * (1-DET) / (1+EPS*X)**2
            FAVG2_GRID = 1 / SUMDPSI
            FAVG2A_GRID = 1 / SUMDPSI * RHO
            FAVG2B_GRID = (1+EPS*X)**2 / SUMDPSI * RHO
            FAVG2C_GRID = 1 / SUMDPSI * RHO * DWDPSI
            FAVG3_GRID = 1 / (1+EPS*X)**2 / (1-DET) 
     >                    / SUMDPSI

!     Calculate q-profile from grid, so that FDF can be calculated for output.
      SUMQR_SP(JNGR) = SUMQR_SP(JNGR) + PSRR * XJAC
     >                / ((PSIR**2)*BIGR)* WS    /  (1-DET)
      SUMQR_SP(JNGR) = SUMQR_SP(JNGR) - ER / (BIGR*PSIR)   * WS
     >                    /  (1-DET)
      SUMQR_SP(JNGR) = SUMQR_SP(JNGR) + XJAC*EPS*XR/((BIGR**2)*PSIR)
     >                  * WS  /  (1-DET) 
!       SUMQR_SP(JNGR) = SUMQR_SP(JNGR) - DETR*XJAC
!      >          / (BIGR*DABS(PSIR))/(1-DET)**2    * WS


            FAVG1_SP(JNGR) = FAVG1_SP(JNGR) + 
     >            FAVG1_GRID * XLEN * WS
            FAVG2_SP(JNGR) = FAVG2_SP(JNGR) + 
     >            FAVG2_GRID * XLEN * WS
            FAVG2A_SP(JNGR) = FAVG2A_SP(JNGR) + 
     >            FAVG2A_GRID * XLEN * WS
            FAVG2B_SP(JNGR) = FAVG2B_SP(JNGR) + 
     >            FAVG2B_GRID * XLEN * WS
            FAVG2C_SP(JNGR) = FAVG2C_SP(JNGR) + 
     >            FAVG2C_GRID * XLEN * WS
            FAVG3_SP(JNGR) = FAVG3_SP(JNGR) + 
     >            FAVG3_GRID * XLEN * WS
            PSAVG(JNGR) = PSAVG(JNGR) +
     >            PS * XLEN * WS

 31   CONTINUE
 21   CONTINUE
      ! IF (NMG*NI.NE.1) PSAVG(NR+1-J) = ZPSI
      ! PSAVG(NR+1-J) = PSI(4*((J-1)*NP)+1)

      PSAVG(JNGR) = DABS(PSAVG(JNGR))  !Most recent
     >       / DABS(BOUNDLEN)

      PSAVG(JNGR) = PS

      FAVG1_SP(JNGR) = DABS(FAVG1_SP(JNGR))
      FAVG2_SP(JNGR) = DABS(FAVG2_SP(JNGR))
      FAVG2A_SP(JNGR) = DABS(FAVG2A_SP(JNGR))
      FAVG2B_SP(JNGR) = DABS(FAVG2B_SP(JNGR))
      FAVG2C_SP(JNGR) = DABS(FAVG2C_SP(JNGR))
      FAVG3_SP(JNGR) = DABS(FAVG3_SP(JNGR))
      DFAVGDPSI_SP(JNGR) = DABS(FAVG1_SP(JNGR) * FAVG3_SP(JNGR) / 
     >       QVAL(PSAVG(JNGR)))

C-----Calculate the un-normalised F-profile from the integral FAVG3
C-----and the q-profile QVAL.
      FPS_SP(JNGR) = PI / EPS * QVAL(PSAVG(JNGR))
     >    / FAVG3_SP(JNGR) /0.5/FACTAS


c-----Calculate FDF for later integration
!       FDF_SP(JNGR) = -B *((DTE + DH*HOT)*FAVG2A_SP(JNGR)
!      >      +DOMG1*OMGOT*0.5*FAVG2B_SP(JNGR) - FAVG2C_SP(JNGR)
!      >      / FAVG3_SP(JNGR) - FAVGVAL(PS,5) / FAVG3_SP(JNGR))
      FDF_SP(JNGR) = -B *(DTE*FAVG2_SP(JNGR)
     >      / FAVG3_SP(JNGR)) - FAVGVAL(PS,5) / FAVG3_SP(JNGR)
...
c-----Calculate DF (using FDF for output instead)
      DF_SP(JNGR) = XGAMMA(ZPSI) * SUMQR_SP(JNGR) - DQ
     > * 2 * PI * DSQRT(ZPSI) / (NR-1) / FACTAS / EPS 
      DF_SP(JNGR) = DF_SP(JNGR) / PSIR / FAVG3_SP(JNGR)

      IF (JNGR.EQ.1) THEN
           DQ = 0.D0
           DF_SP(1) = XGAMMA(0.D0) * SUMQR_SP(JNGR) 
     >      / PSIR / FAVG3_SP(JNGR)
      ENDIF
      ...
      ! DQS(NR-I+1)=  0.5*FACTAS * DQS(NR-I+1) * EPS
      ! DQS(NR-I+1)=  DQS(NR-I+1) * DSQRT(DABS(A))
      DQS = SUMQR_SP(JNGR)*XGAMMA(ZPSI) - FAVG3_SP(JNGR)*DGDPSI(ZPSI)
     > * PSIR
      DQS =  0.5*FACTAS * DQS * EPS
      DQS =  DQS * 2 * (NR-1) / PI
      DQS = DQS / 2 / SQRT(ZPSI)

      DF2 = XGAMMA(ZPSI) * SUMQR_SP(JNGR) - DQS
     > * 2 * PI * DSQRT(ZPSI) / (NR-1) / FACTAS / EPS 
      DF2 = DF2 / PSIR / FAVG3_SP(JNGR)


!       WRITE(*,*) ZPSI,FPS_SP(JNGR), XGAMMA(ZPSI), DF_SP(JNGR), DF2,
!      > DGDPSI(PSAVG(JNGR)), DQS * 2 * DSQRT(ZPSI),
!      >       DQ * 2 * DSQRT(ZPSI)
!       WRITE(*,*) PSAVG(JNGR), SUMQR_SP(JNGR),
!      >      DQDPSI(ZPSI)*DSQRT(ZPSI)*2, DF_SP(JNGR),
!      >      DGDPSI(PSAVG(JNGR)), XGAMMA(ZPSI), NR
!       WRITE(*,*) PSAVG(JNGR), DQS * 2 * DSQRT(ZPSI),
!      >       DQ * 2 * DSQRT(ZPSI)
      ! WRITE(*,*) PS, PS1, PSIR, PSIR1, PSRR, PSIRR1
...
C-----Remove elements too near the core
c-----Cut off the core
      ! IF (PSAVG(NR+1-J).GE.2.D-1) THEN
      !       FAVG1_SP2(J2) = FAVG1_SP(NR+1-J)
      !       FAVG2_SP2(J2) = FAVG2A_SP(NR+1-J)
      !       FAVG3_SP2(J2) = FAVG3_SP(NR+1-J)
      !       DFAVGDPSI_SP2(J2) = DFAVGDPSI_SP(NR+1-J)
      !       PSAVG4(J2) = PSAVG(NR+1-J)
      !       J2 = J2+1
      ! ENDIF

      ! IF (NI*NMG.EQ.1) THEN
      !       FPS2(J1) = FPS_SP(NR+1-J)
      !       PSAVG2(J1) = PSAVG(NR+1-J)
      !       J1 = J1+1
      ! ELSEIF ((PSAVG(NR+1-J).GE.1.D-1).AND.(NI*NMG.NE.1)) THEN
      !       FPS2(J1) = FPS_SP(NR+1-J)
      !       PSAVG2(J1) = PSAVG(NR+1-J)
      !       J1 = J1+1
      ! ENDIF

C-----DELTA LOOP intended to find DELTA on each Q-SOLVER LOOP.
C-----From testing, DELTA seems to converge without this, so it is skipped.
      GOTO 10 !skip delta-loop.
      ! IF ((NMG*NI).EQ.1) GOTO 10 !skip delta-loop.
C-----------------WITHOUT ANISOTROPY
      IF ((ITH.LT.1).OR.(THTOF.EQ.0)) THEN
            ! WRITE(*,*) 'SKIP DELTA LOOP'
            GOTO 10 !skip delta-loop.
      ENDIF
C-----------------WITH ANISOTROPY
      IF (THTOF.GT.0.D0) THEN
      ...
      !!!Use FAVG3 to calc BTOT, TPER, RHO, F over grid.
      DO 22 I = 1, NP-1
            N1 = (J-1)*NP + I
            N2 = (J-1)*NP + I + 1
            N3 = (J-1)*NP + I + 1 + NP
     ...
     >            PSI(4*(N4-1)+1),R,S,X,XR,XS,YR,YS,PS)
            ...
            IF (k.EQ.1) THEN
                  DET = 0.D0
c-----Determine initial BM0 with DET=0
                  BTOT = DSQRT(DABS(A)*FPS**2/(1-DET)**2 + SUMDPSI**2)
     >        /(1+EPS*X)
                  TPER = BTOT*TEMP /
     >        DABS(BTOT - DSQRT(DABS(A)) * TEMP * THETAPS * THTOF)
                  BM0 = DET - TPER/TEMP * 
     >        RHO*DABS(A)*B*(TEMP - TPER)/BTOT**2
                  BM0P(4*(I-1)+NGS) = BM0
                  DET = -0.3D0
            ELSE 
                  DET = -0.3D0
c-----Determine initial BM3 with DET=0
                  BTOT = DSQRT(DABS(A)*FPS**2/(1-DET)**2 + SUMDPSI**2)
     >        /(1+EPS*X)
                  TPER = BTOT*TEMP /
     >        DABS(BTOT - DSQRT(DABS(A)) * TEMP * THETAPS * THTOF)
...
c-----Calculate FPS for DET=-0.3 then DET=-1/1000
                  TPAR = TEMS(PS)
                  RHO = TPER / TPAR * CALCRHO(X, PS)
                  PPER = RHO * TPER * A * B
                  PPAR = RHO * TPAR * A * B
...
c-----Skip first 1:1000 if binary bounds are good
      IF (K.EQ.1) GOTO 50
      DO K1=1,(NP-1)*4
            BM0=BM0P(K1)
            BM3=BM3P(K1)
            IF (BM0*BM3.GE.0) GOTO 50
            DETLP(K1) = -0.3
            DETRP(K1) = 0
      ENDDO

      ! WRITE(*,*) J,"FOUND BINARY DET"
      
      ! GOTO 52


c-----Determine binary values for DETL DETR
      ! IF ((K.EQ.2).AND.(BM3*BM0.LT.0)) THEN
      ! DETLP(4*(I-1)+NGS) = -0.3
      ! DETRP(4*(I-1)+NGS) = 0
      ! ! GOTO (51??? skip first 1000 loop?) -> leave for now.  Work out
      ! ! a surface-based condition.
      ! ENDIF

 50   CONTINUE


C----------- Begin looping over DELTA.  Go first 1:1000 
C----------- Search until bounds for DETL and DETR are found
      
      DO 51 K = 1, 1000
            ! DET = DFLOAT(K)/1000.D0 * (-1)
...
c-----Check DET for grid-point
                  IF (BM*BM0.LT.0) THEN
                        !!update for grid point, then goto only if all points converged.
                        DETLP(4*(I-1)+NGS) = DETP(4*(I-1)+NGS)
                        DETRP(4*(I-1)+NGS) = 0
                  ELSE
                        DET = DFLOAT(K+1)/1000.D0 * (-1)
                        DETP(4*(I-1)+NGS) = DET
                        !If no DETR=-99 before K=1000, then continue
                        !else, STOP
                        DETLP(4*(I-1)+NGS) = 0
                        DETRP(4*(I-1)+NGS) = -99
                  ENDIF

c-----Calculate FPS for next DET
                  TPAR = TEMS(PS)
                  RHO = TPER / TPAR * CALCRHO(X, PS)
                  PPER = RHO * TPER * A * B
                  PPAR = RHO * TPAR * A * B
...
!!!!exit point for first 1:1000
      DO K1=1,(NP-1)*4
            IF (DETRP(K1).EQ.-99) GOTO 51
      ENDDO
      GOTO 52
...
c ------------ update BL and BR for each grid point before next loop.
      DO 53 k=1,2
...
c-----Determine BML and BMR
     ...
     >        RHO*DABS(A)*B*(TEMP - TPER)/BTOT**2
                  BRP(4*(I-1)+NGS) = BR
                  DET = (DETLP(4*(I-1)+NGS)+DETRP(4*(I-1)+NGS))/2
                  DETP(4*(I-1)+NGS) = DET
            ENDIF
...
c------------ second Do 1:1000.  Fine search for DELTA using BR and BL.
...
c-----Check DET for grid-point
                  ...
                  DETL=DETLP(4*(I-1)+NGS)
                  DETR=DETRP(4*(I-1)+NGS)
                  DET = 0.5*(DETL+DETR)
                  DETP(4*(I-1)+NGS) = DET
...
C-----and the q-profile QVAL.
     ...
     >    / FAVG3_SP(JNGR) /0.5/FACTAS
      ...
      ! WRITE(*,*) K, 4*(I-1)+NGS,DETL,DETR

!!!!exit point for first 1:1000
      DO K1=1,(NP-1)*4
            DETL=DETLP(K1)
            DETR=DETRP(K1)
            IF (DABS(DETL-DETR).GT.1.D-12) THEN
                  GOTO 54
            ENDIF
      ENDDO
      ! WRITE(*,*) J, "FOUND DELTA SOLUTION"
      GOTO 10
 ...
 10   CONTINUE
      ...
      ! DO K1=1,(NP-1)*4
      !       DETR=DETRP(4*(I-1)+NGS)
      !       IF (DABS(DETR).EQ.0.8D0) STOP 'MIRROR INSTABILITY OCCURS'
      ! ENDDO

c-----spline favg values to NPTS = 1001.
      ! FAVG1_SP(1) = 0.D0
      ! DFAVGDPSI_SP(1) = 0.D0
      ! PSAVG(1) = 0.D0

!       IF (NMG.LE.NMESH) THEN
!             FAVG2_SP(1) = FAVG2_SP(2) - PSAVG(2) * 
!      >             (FAVG2_SP(3)-FAVG2_SP(2)) / (PSAVG(3)-PSAVG(2))
!             FAVG2A_SP(1) = FAVG2A_SP(2) - PSAVG(2) * 
!      >             (FAVG2A_SP(3)-FAVG2A_SP(2)) / (PSAVG(3)-PSAVG(2))
!             FAVG2B_SP(1) = FAVG2B_SP(2) - PSAVG(2) * 
!      >             (FAVG2B_SP(3)-FAVG2B_SP(2)) / (PSAVG(3)-PSAVG(2))
!             FAVG2C_SP(1) = FAVG2C_SP(2) - PSAVG(2) * 
!      >             (FAVG2C_SP(3)-FAVG2C_SP(2)) / (PSAVG(3)-PSAVG(2))
!             FAVG3_SP(1) = FAVG3_SP(2) - PSAVG(2) * 
!      >             (FAVG3_SP(3)-FAVG3_SP(2)) / (PSAVG(3)-PSAVG(2))
!             FPS_SP(1) = PI / EPS * QVAL(0.D0) 
!      >    / FAVG3_SP(1) /0.5/FACTAS
!       ELSE
!             FAVG1_SP(1) = 0.D0
!             DFAVGDPSI_SP(1) = 0.D0
!             PSAVG(1) = 0.D0
...
c-----Cut-off the core option
!       DO I=2,NR
!       IF (I.LE.NR-J2+1) THEN
!             FAVG1_SP3(I) = FAVG1_SP3(1) + PSAVG(I) *
!      >             (FAVG1_SP2(J2-1) - FAVG1_SP3(1)) 
!      >             / PSAVG4(J2-1)
!             FAVG2_SP3(I) = FAVG2_SP3(1) + PSAVG(I) *
!      >             (FAVG2_SP2(J2-1) - FAVG2_SP3(1)) 
!      >             / PSAVG4(J2-1)
!             FAVG3_SP3(I) = FAVG3_SP3(1) + PSAVG(I) *
!      >             (FAVG3_SP2(J2-1) - FAVG3_SP3(1)) 
!      >             / PSAVG4(J2-1)
!             DFAVGDPSI_SP3(I) = DFAVGDPSI_SP3(1) + PSAVG(I) *
!      >             (DFAVGDPSI_SP2(J2-1) - DFAVGDPSI_SP3(1)) 
!      >             / PSAVG4(J2-1)
!             PSAVG5(I) = PSAVG(I)
!       ELSE
!             FAVG1_SP3(I) = FAVG1_SP2(NR-I+1)
!             FAVG2_SP3(I) = FAVG2_SP2(NR-I+1)
!             FAVG3_SP3(I) = FAVG3_SP2(NR-I+1)
!             DFAVGDPSI_SP3(I) = DFAVGDPSI_SP2(NR-I+1)
!             PSAVG5(I) = PSAVG4(NR-I+1)
!       ENDIF
!       ENDDO

!       WRITE(*,*) 'on axis:', FAVG1_SP(1), FAVG2_SP(1), FAVG3_SP(1),
!      >             DFAVGDPSI_SP(1)
!       DO J=1,JMAX
!             WRITE(60,*) PSAVG(J), FAVG1_SP(J), FAVG2_SP(J),
!      >            FAVG3_SP(J), FAVG2A_SP(J), FAVG2B_SP(J),
!      >            FAVG2C_SP(J), QVAL(PSAVG(J)), DFAVGDPSI_SP(J), 
!      >            FPS_SP(J), NI, NMG
!       ENDDO



C-----Store last iteration of flux-variables
      DO I=1,JMAX
            PS = PSAVG(I)
            F1_OLD(I) = FAVGVAL(PS,1)
            F2_OLD(I) = FAVGVAL(PS,2)
            F3_OLD(I) = FAVGVAL(PS,3)
            F2A_OLD(I) = FAVGVAL(PS,6)
            F2B_OLD(I) = FAVGVAL(PS,7)
            F2C_OLD(I) = FAVGVAL(PS,8)
!             WRITE(*,*) PS, F1_OLD(I),
!      >                   FAVG1_SP(I)
      ENDDO

      ! CALL ININUM(1001,4*NR,XBOUNDLEN,DVDPSI,VDEV_TEMP)
      CALL INIFQ(JMAX,1001,PSAVG,FAVG1_SP,FAVG1,DFAVG1)
      CALL INIFQ(JMAX,1001,PSAVG,FAVG2_SP,FAVG2,VDEV_TEMP)
      CALL INIFQ(JMAX,1001,PSAVG,FAVG2A_SP,FAVG2A,VDEV_TEMP)
      CALL INIFQ(JMAX,1001,PSAVG,FAVG2B_SP,FAVG2B,VDEV_TEMP)
      CALL INIFQ(JMAX,1001,PSAVG,FAVG2C_SP,FAVG2C,VDEV_TEMP)
      CALL INIFQ(JMAX,1001,PSAVG,FAVG3_SP,FAVG3,VDEV_TEMP)
      CALL INIFQ(JMAX,1001,PSAVG,DFAVGDPSI_SP,VDEV_TEMP,DFAVGDPSI)

C-----Check difference in fllux variables
      RES1 = 0
      RES2 = 0
      RES3 = 0
      RES2A = 0
      ...
      DO I=1,JMAX
            PS = PSAVG(I)
            RES1 = RES1 + (FAVGVAL(PS,1)-F1_OLD(I))**2
            RES2 = RES2 + (FAVGVAL(PS,2)-F2_OLD(I))**2
            RES3 = RES3 + (FAVGVAL(PS,3)-F3_OLD(I))**2
            RES2A = RES2A + (FAVGVAL(PS,6)-F2A_OLD(I))**2
            RES2B = RES2B + (FAVGVAL(PS,7)-F2B_OLD(I))**2
            RES2C = RES2C + (FAVGVAL(PS,8)-F2C_OLD(I))**2
!             WRITE(*,*) PS, FAVGVAL(PS,1), F1_OLD(I),
!      >                   FAVG1_SP(I)
      ENDDO
      RES1 = DSQRT(RES1)
      RES2 = DSQRT(RES2)
      ...
      RES2C = DSQRT(RES2C)
      IF ((ITH.LT.1).OR.(THTOF.EQ.0)) THEN
            RES_CONV = MAX(RES1,RES2,RES3)
      ELSE
            RES_CONV = MAX(RES1,RES2,RES3,RES2A,RES2B,RES2C)
      ENDIF
...
C-----Sort F and psi by ascending psi------------------------------------
      ! CALL BUBBLESORT(NR,PSAVG,FPS_SP)

! c----apply limited F profile
!       NR2 = J1
!       FPS3(1) = -999.0
!       PSAVG3(1) = 0.0

!       DO I=2,NR
!       IF (I.LE.NR-NR2+1) THEN
!             FPS3(I) = FPS0 + (FPS2(NR2-1) - FPS0) 
!      >        * PSAVG(I) / PSAVG2(NR2-1)
!             PSAVG3(I) = PSAVG(I)
!       ELSE
!             FPS3(I) = FPS2(NR-I+1)
!             PSAVG3(I) = PSAVG2(NR-I+1)
!       ENDIF
!       ENDDO

!       CALL BUBBLESORT(NR,PSAVG3,FPS3)
!       IF (NI*NMG.NE.1) THEN
!             FPS3(1) = FPS0
!       ELSE
!             FPS3(1) = FPS3(2) - (FPS3(3)-FPS3(2)) / 
!      >            (PSAVG3(3)-PSAVG3(2)) * PSAVG3(2)
!       ENDIF
!       FPS3 = FPS3 / FPS3(1)

C-----SECTION FOR F-calculation.  Skip for diagnostics
      ! IF (IGAM.EQ.5) CALL INIFQ(JMAX,1001,PSAVG,FPS_SP,GINT,DGAM)
      IF ((IGAM.EQ.5).AND.(FG_CHECK.EQ.0)) THEN
            ! FPS0 = FPS_SP(1)
            ! CALL INIFQ(JMAX,1001,PSAVG,FPS_SP/FPS0,GINT,DGAM)
            ! GINT = GINT*FPS0
            CALL INIFQ(JMAX,1001,PSAVG,FPS_SP,GINT,DGAM)
            CALL INIFQ(JMAX,1001,PSAVG,2*FDF_SP,DF2AVG,VDEV_TEMP)
      ENDIF
      ! IF (NI*NMG.NE.1) FG_CHECK = 0
      FG_CHECK = 0



!       WRITE(*,*) 'TEST OUTPUT FOR QSOLVER'
!       DO 40 J = NR, 1, -1
!             PS = PSI(4*((J-1)*NP)+1)
!             WRITE(*,*) PS, FAVGVAL(PS,1), FAVGVAL(PS,2), FAVGVAL(PS,3),
!      >                  FAVGVAL(PS,4)
            ! WRITE(*,*) PS, XGAMMA(PS), DGDPSI(PS)
!  40   CONTINUE
!       DO 41 J = 1,NR
!             PS = PSI(4*((J-1)*NP)+1)
!             ! WRITE(*,*) PSAVG3(J), FPS3(J)
!  41   CONTINUE      
      ! DO I=1,JMAX
      !       PS = PSAVG(I)
      !       WRITE(*,*) PS, XGAMMA(PS), DGDPSI(PS)
      ! ENDDO
      WRITE(*,*) 'q-solver terms calculated'
      RETURN
      END

************************************************************************
*DECK BUBBLESORT
      SUBROUTINE BUBBLESORT(NPTS,XARRAY,YARRAY)
C-----------------------------------------------------------------------
C BASIC SORT (VERY SLOW ALGORITHM - JUST A PLACEHOLDER)
C SORT XARRAY AND YARRAY BY ASCENDING XARRAY
C-----------------------------------------------------------------------

      INTEGER SWAPPED, NPTS
      DOUBLE PRECISION XARRAY(NPTS), YARRAY(NPTS)

      SWAPPED = 0
      DO WHILE (SWAPPED.EQ.0)
      SWAPPED = 1
      DO I=1,NPTS-1
      IF (XARRAY(I+1).LT.XARRAY(I)) THEN
            X1 = XARRAY(I+1)
            Y1 = YARRAY(I+1)
            XARRAY(I+1) = XARRAY(I)
            XARRAY(I) = X1
            ...
            YARRAY(I) = Y1
            SWAPPED = 0
c-----------check for repeating elements
      ELSEIF (XARRAY(I+1).EQ.XARRAY(I)) THEN
            IF (I.LE.(NPTS-2)) THEN
            XARRAY(I+1) = (XARRAY(I) + XARRAY(I+2)) / 2
            YARRAY(I+1) = YARRAY(I) + (YARRAY(I+2) - YARRAY(I)) 
     >       / (XARRAY(I+2) - XARRAY(I)) * (XARRAY(I+1) - XARRAY(I))
            ELSE
            XARRAY(I) = (XARRAY(I-1) + XARRAY(I+1)) / 2
            YARRAY(I) = YARRAY(I-1) + (YARRAY(I+1) - YARRAY(I-1)) 
     >       / (XARRAY(I+1) - XARRAY(I-1)) * (XARRAY(I) - XARRAY(I-1))   
            ENDIF
            SWAPPED = 0
      ENDIF
      ENDDO
...
!      >               ERR,XL(I),BUSSAC,2*PI*ZVOL(NR-I+1),
!      >               2*PI*ZVOLP(NR-I+1),ZAR(I)
      WRITE(20,72) I,ZPS(I),SQRT(ZPS(I)),AVC(I)/AVC(NR),
     >               0.D0,0.D0,0.D0,2*PI*ZVOL(NR-I+1),
     >               2*PI*ZVOLP(NR-I+1),0.D0
   70 CONTINUE
      WRITE(20,*) '***********************************************'
     >             //'********'
      WRITE(20,*)
...
C------------------------------------- 4 POINT GAUSSIAN INT. IN S -----
     ...
     >                  R,S,PS,PSR,PSS)
            ...
            ARHS =  ARHS / (1.+EPS*X)
C---------------------------------------------- SOLOVIEV RHS ----------
            IF (ISOL.GE.1) ARHS = (1.+ B*X*(1.+EPS*X/2.))/(1.+EPS*X)
            CAREA = CAREA + WR * WS * XJAC * ARHS
   30     CONTINUE
   20   CONTINUE
   15   CONTINUE
